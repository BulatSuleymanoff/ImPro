...
class PrewittFilter : public LinearFilter
{
public:
	PrewittFilter() : LinearFilter()
	{
		axis_ = 1;
		kernel_[0][0] = -1.;
		kernel_[0][1] = 0.;
	...
	PrewittFilter(string input_path, bool mode) : LinearFilter(input_path, mode)
	{
		axis_ = 1;
	...
	PrewittFilter(bool axis) : LinearFilter()
	{
		axis_ = axis;
		if (axis == X_AXIS) {
			kernel_[0][0] = -1.;
	...
	PrewittFilter(string input_path, bool mode, bool axis) : LinearFilter(input_path, mode)
	{
		axis_ = axis;
		if (mode == NOT_SEPARABLE) {
			if (axis == X_AXIS) {
